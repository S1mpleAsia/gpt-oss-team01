...
void our_init_weights(TransformerWeights *w, Config *p, OurTransformerWeights *weights) {
    ...
    weights->token_embedding_table = new Tensor({(size_t)p->vocab_size, (size_t)p->hidden_dim}, w->token_embedding_table);
    ...
    weights->w_qkv = new Tensor({(size_t)p->n_layers, ((size_t)p->n_attn_heads + 2 * (size_t)p->n_kv_heads) * (size_t)p->head_dim, (size_t)p->hidden_dim}, w->w_qkv);
    weights->b_qkv = new Tensor({(size_t)p->n_layers, ((size_t)p->n_attn_heads + 2 * (size_t)p->n_kv_heads) * p->head_dim}, w->b_qkv);

    weights->w_o = new Tensor({(size_t)p->n_layers, (size_t)p->hidden_dim, (size_t)p->n_attn_heads * p->head_dim}, w->w_o);
    ...
    weights->w_router = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, (size_t)p->hidden_dim}, w->w_router);
    weights->b_router = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts}, w->b_router);
    
    weights->w_mlp1 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, 2 * (size_t)p->intermediate_dim, (size_t)p->hidden_dim}, w->w_mlp1, DType::BF16);
    weights->b_mlp1 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, 2 * (size_t)p->intermediate_dim}, w->b_mlp1, DType::BF16);
    
    weights->w_mlp2 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, (size_t)p->hidden_dim, (size_t)p->intermediate_dim}, w->w_mlp2, DType::BF16);
    weights->b_mlp2 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, (size_t)p->hidden_dim}, w->b_mlp2, DType::BF16);
...
void our_init_run_state(RunState *s, Config *p, OurRunState *rs) {
    ...
    rs->key_cache = new Tensor({(size_t)p->n_layers, (size_t)p->seq_len, (size_t)p->n_kv_heads * p->head_dim}, s->key_cache);
    rs->value_cache = new Tensor({(size_t)p->n_layers, (size_t)p->seq_len, (size_t)p->n_kv_heads * p->head_dim}, s->value_cache);
...
void our_free(OurTransformerWeights *weights, OurRunState *rs) {
    ...
    delete weights->rms_attn_w; delete weights->rms_ffn_w;
    delete weights->w_qkv; delete weights->w_o;
    delete weights->b_qkv; delete weights->b_o;
    delete weights->attn_sinks;
    delete weights->w_router; delete weights->b_router;
    delete weights->w_mlp1; delete weights->w_mlp2;
    delete weights->b_mlp1; delete weights->b_mlp2;
    delete weights->rms_out_w; delete weights->out;

    // delete rs
    delete rs->x; delete rs->t;
    delete rs->tb; delete rs->tb2;
    delete rs->router_score;
    delete rs->topk_v; delete rs->topk_i;
    delete rs->mlp1_out;
    delete rs->gate; delete rs->up;
    delete rs->gate_up; delete rs->e_agg;
    delete rs->qkv; delete rs->q;
    delete rs->k; delete rs->v;
    delete rs->att; delete rs->logits;
    delete rs->key_cache; delete rs->value_cache;
    delete rs->mask;
