...
void our_init_weights(TransformerWeights *w, Config *p, OurTransformerWeights *weights) {
    ...
    weights->w_mlp1 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, 2 * (size_t)p->intermediate_dim, (size_t)p->hidden_dim}, w->w_mlp1);
    weights->b_mlp1 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, 2 * (size_t)p->intermediate_dim}, w->b_mlp1);
    
    weights->w_mlp2 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, (size_t)p->hidden_dim, (size_t)p->intermediate_dim}, w->w_mlp2);
    weights->b_mlp2 = new Tensor({(size_t)p->n_layers, (size_t)p->n_experts, (size_t)p->hidden_dim}, w->b_mlp2);
