...
template <int BM = 16, int BN = 128, int BK = 16, int TM = 2, int TN = 8>
__global__ void matmul_kernel_bf16(
  const float *__restrict__ A, const bf16 *__restrict__ B,
  float *__restrict__ C, const bf16 *bias, int M, int N, int K
);

static inline void moe_init_buffers(
  Tensor *e_agg, Tensor *mlp1_out, Tensor *gate_up, Tensor *tb3,
...
static inline int moe_get_max_rows_per_expert(
  TensorI32 *expert_offsets, int n_experts, hipStream_t stream
);
...
static inline void moe_mlp1_forward(
...
static inline void moe_mlp2_forward(
  Tensor *gate_up,  // [total_pairs, inter_dim]
