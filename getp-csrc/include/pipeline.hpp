...
struct PipelineEach {
  ...
  PipelineEach(const vector<size_t> &shape_, hipStream_t stream, int gpu_id, int size_buf_,
               DType::Type dtype = DType::FP32);
  ~PipelineEach();

  void enqueueElem(Tensor *x, int sent_gpu_id, hipEvent_t x_ready, hipStream_t recv_stream);
