...
void single_query_attn_flash_batched_opt(
  ...
  const int * /*layer_tokens*/, const uint8_t * /*layer_is_window*/,
  ...
  if (L_eff > FA_TILE) {
    ...
    fd_partial_kernel_bf16_grouped<FA_TILE, FA_THREADS, 64, GROUP_SIZE, ELEMS_PER_LOAD>
      <<<grid_p, block_p, shmem_p, stream>>>((const float *)q->d_buf, K_ptr, V_ptr, d_pmax, d_psum,
                                             d_pnum, num_query_heads, kv_mul, kv_dim, (int)n_layers,
    ...
    fd_reduce_kernel_stable_v2<64><<<grid_r, block_r, 0, stream>>>(
      d_pmax, d_psum, d_pnum, S_ptr, (float *)tb->d_buf, num_query_heads, chunk, cur_batch_size);
  }
