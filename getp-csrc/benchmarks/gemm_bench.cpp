...
// ----------------- Utils -----------------
struct GemmProblem { int M, N, K; };

static void fill_random_normal(std::vector<float> &dst, float mean, float stddev) {
  std::mt19937 gen(42);
  std::normal_distribution<float> dist(mean, stddev);
  for (float &v : dst) v = dist(gen);
}

static inline bf16 f32_to_bf16(float v) { return bf16(v); }
...
static float rel_l2_err(const std::vector<float>& a, const std::vector<float>& b) {
  ...
  for (size_t i=0;i<a.size();++i) { long double d=(long double)a[i]-b[i]; num+=d*d; den+=(long double)b[i]*b[i]; }
  return den>0 ? (float)std::sqrt((double)(num/den)) : 0.f;
}

// ----------------- bf16 -> f32 conversion kernel -----------------
__global__ void bf16_to_fp32_kernel(const bf16* __restrict__ bf16_in,
                                    float* __restrict__ fp32_out,
                                    size_t n) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  size_t s = gridDim.x * blockDim.x;
  for (; i < n; i += s) fp32_out[i] = (float)bf16_in[i];
}

// ----------------- Bench: your custom kernel -----------------
static float bench_custom(const float* dA_f32, const bf16* dB_bf16, float* dC,
                          int M, int N, int K, hipStream_t stream, int iters) {
  CHECK_HIP(hipMemsetAsync(dC, 0, sizeof(float)*(size_t)M*N, stream));
  // warmup
  for (int i=0;i<3;++i) launch_gemm_mfma_v2(dA_f32, dB_bf16, dC, M,N,K, stream);
  CHECK_HIP(hipStreamSynchronize(stream));
  // timed
  hipEvent_t start, stop; CHECK_HIP(hipEventCreate(&start)); CHECK_HIP(hipEventCreate(&stop));
  CHECK_HIP(hipEventRecord(start, stream));
  for (int i=0;i<iters;++i) launch_gemm_mfma_v2(dA_f32, dB_bf16, dC, M,N,K, stream);
  CHECK_HIP(hipEventRecord(stop, stream)); CHECK_HIP(hipEventSynchronize(stop));
  float ms=0.f; CHECK_HIP(hipEventElapsedTime(&ms, start, stop));
...
static float bench_rocblas_sgemm_rowmajor(const float *dA_r, const bf16 *dB_r_bf16, float *dC_r,
  ...
  hipLaunchKernelGGL(bf16_to_fp32_kernel, dim3(blocks), dim3(tpb), 0, stream,
                     dB_r_bf16, dB_r_f32, size_b);
  CHECK_HIP(hipStreamSynchronize(stream));

  const float alpha=1.f, beta=0.f;
  auto sgemm_nn = [&]() {
    return rocblas_sgemm(handle,
  ...
  // warmup
  for (int i=0;i<3;++i) CHECK_ROCBLAS(sgemm_nn());
  CHECK_HIP(hipStreamSynchronize(stream));

  // timed
  hipEvent_t start, stop; CHECK_HIP(hipEventCreate(&start)); CHECK_HIP(hipEventCreate(&stop));
  CHECK_HIP(hipEventRecord(start, stream));
  for (int i=0;i<iters;++i) CHECK_ROCBLAS(sgemm_nn());
  CHECK_HIP(hipEventRecord(stop, stream)); CHECK_HIP(hipEventSynchronize(stop));
  float ms=0.f; CHECK_HIP(hipEventElapsedTime(&ms, start, stop));
...
int main(int argc, char **argv) {
  ...
  if (argc==1) problems.push_back({2048,5760,2880});
  else {
    for (int i=1;i<argc;i+=3) {
      GemmProblem p{std::atoi(argv[i]), std::atoi(argv[i+1]), std::atoi(argv[i+2])};
      if (p.M<=0||p.N<=0||p.K<=0) { std::cerr<<"Invalid dims\n"; return EXIT_FAILURE; }
  ...
  for (auto &pb : problems) {
    ...
    for (size_t i=0;i<sizeB;++i) hB_bf16[i] = f32_to_bf16(hB_f32[i]);
    ...
    CHECK_HIP(hipMemcpyAsync(dB, hB_bf16.data(), sizeof(bf16)*sizeB, hipMemcpyHostToDevice, stream));
    CHECK_HIP(hipStreamSynchronize(stream));
...
#if WITH_ROCBLAS
    CHECK_HIP(hipMemcpy(hC_rocblas.data(), dC_rocblas, sizeof(float)*sizeC, hipMemcpyDeviceToHost));
#endif

    const double gflops = (2.0 * (double)M * N * K) * 1e-6;
    std::cout << "  gemm_mfma_v2            : " << ms_custom
              << " ms  (" << (gflops / ms_custom) << " GFLOP/s)\n";
#if WITH_ROCBLAS
    std::cout << "  rocBLAS (SGEMM NN map)  : " << ms_rocblas
              << " ms  (" << (gflops / ms_rocblas) << " GFLOP/s)\n";
