...
void *thread_handler_20b(void *arg) {
  ...
  args->tp_barrier = new pthread_barrier_t[PP_20B * 13];

  CHECK_HIP(hipSetDevice(id));

  for (int cur_idx = args->start_idx; cur_idx < end_idx; cur_idx += BATCH_SIZE_20B) {
    int current_size = min(BATCH_SIZE_20B, end_idx - cur_idx);
...
void *thread_handler_120b(void *arg) {
  ...
  int max_seq_len = public_requests->max_seq_len;
  int end_idx = args->end_idx;
  args->tp_barrier = new pthread_barrier_t[PP_120B * 13];
  ...
  for (int cur_idx = args->start_idx; cur_idx < end_idx; cur_idx += BATCH_SIZE_120B) {
    int current_size = min(BATCH_SIZE_120B, end_idx - cur_idx);
    ...
    for (int pos = 0; pos < max_seq_len - 1 && active_count > 0; ++pos) {
      ...
      int *batch_tokens = inside_args[(PP_120B - 1) * TP_120B].tokens;
