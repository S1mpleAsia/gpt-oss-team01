...
void MemCpy_Tensor(Tensor *to, Tensor *from, long long to_offset, long long from_offset, size_t num_elem, bool copy_host, bool copy_device) {
    ...
    if (copy_device) {
        if (to->dtype == DType::BF16) {
            ...
            CHECK_HIP(hipMemcpy(to_d_buf_bf16, from_d_buf_bf16, num_elem * sizeof(bf16), hipMemcpyDeviceToDevice));
        } else {
            float *to_d_buf_fp32 = (float *)to->d_buf + 1ll * to_offset;
            float *from_d_buf_fp32 = (float *)from->d_buf + 1ll * from_offset;
        
            CHECK_HIP(hipMemcpy(to_d_buf_fp32, from_d_buf_fp32, num_elem * sizeof(float), hipMemcpyDeviceToDevice));
        }
...
void MemSet_Tensor(Tensor *in, int value, bool set_host, bool set_device) {
    ...
    if (set_host) {
        ...
        CHECK_HIP(hipMemset(in->d_buf, 0, in->num_elem() * in->get_dtype_size()));
    }
}