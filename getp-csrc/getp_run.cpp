...
void finish(Transformer *transformer, Tokenizer *tokenizer) {
  ...
  /** INSIDE THREAD HANDLER
    void *inside_thread_handler(void *arg) {
      ...
      args->logits = forward_gpu_120b_batched(args->current_tokens[0], args->pos, args->current_size, args->id, args->tp_rank, args->pp_rank, args->tp_barrier);

      return nullptr;
    }
  */

  void *inside_thread_handler_new(void *arg) {
    OnePathInsideArgs *args = (OnePathInsideArgs *)arg;
    ...
    pthread_barrier_wait(args->debug_barrier);

    for (int loop_id = 0; loop_id < args->num_loops; loop_id++) {
      WorkItem *cur_work_item = args->work_items[loop_id];

      for (int pos = 0; pos < max_seq_len && cur_work_item->active_count > 0; ++pos) {
        float *logits = forward_gpu_120b_batched(
          args->work_items[loop_id]->current_tokens, pos,
          args->work_items[loop_id]->current_size, args->id,
          args->tp_rank, args->pp_rank, args->tp_barrier
        );
        
        pthread_barrier_wait(args->debug_barrier);

        if (args->tp_rank == TP - 1 && args->pp_rank == 0) {
          cur_work_item->nextStep(public_config, logits);
        }
        // all thread waits here
  
        pthread_barrier_wait(args->debug_barrier);
      }

      if (args->tp_rank == TP - 1 && args->pp_rank == 0) {
        cur_work_item->retrieveResults(args->total_generate_tokens);
      }
  ...
  void *thread_handler(void *arg) {
    ...
    pthread_barrier_init(&debug_barrier, NULL, TOTAL_PIPELINES);

    
    int num_loops = (end_idx - args->start_idx + BATCH_SIZE - 1) / BATCH_SIZE;
    *(args->local_token_ptr) = 0ll;
    WorkItem **work_items = new WorkItem*[num_loops];
    int work_item_id = 0;
    ...
    for (int cur_idx = args->start_idx; cur_idx < end_idx; cur_idx += BATCH_SIZE) {
      ...
      // Allocate a new WorkItem object and assign the pointer to the array
      work_items[work_item_id] = new WorkItem(current_size);
      work_items[work_item_id]->initItem(public_requests, public_tokenizer, public_config, cur_idx);
      work_item_id++;
    }
    ...
    for (int i = 0; i < TOTAL_PIPELINES; i++) {
      ...
      inside_args[i].work_items = work_items;
      inside_args[i].total_generate_tokens = args->local_token_ptr;
      inside_args[i].num_loops = num_loops;
      inside_args[i].id = id;
      inside_args[i].debug_barrier = &debug_barrier;
    }

    for (int i = 0; i < TOTAL_PIPELINES; i++) {
      pthread_create(&inside_threads[i], NULL, inside_thread_handler_new,
                  (void *)&inside_args[i]);
    }
    ...
    /** ORIGINAL
      for (int loop_id = 0; loop_id < num_loops; loop_id++) {
        WorkItem *cur_work_item = work_items[loop_id];
        for (int i = 0; i < TOTAL_PIPELINES; i++) {
          inside_args[i].current_tokens = &(cur_work_item->current_tokens);
          inside_args[i].current_size = cur_work_item->current_size;
          inside_args[i].id = id;
        }
        ...
        for (int pos = 0; pos < max_seq_len && cur_work_item->active_count > 0; ++pos) {
          ...
          cur_work_item->nextStep(public_config, batch_logits);
        }

        cur_work_item->retrieveResults(&local_token_count);
      }
    */
    ...
    delete args->tp_barrier;// First, loop through the array and delete each individual WorkItem object
    for (int i = 0; i < num_loops; i++) {
      // Check for null pointers to prevent crashes on an incomplete array
      if (work_items[i] != nullptr) delete work_items[i];
    }

    // Second, delete the array of pointers itself
    delete[] work_items;
