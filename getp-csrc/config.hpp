...
typedef struct {
  ...
  // the shape is kinda reverted because the original code use einsum to reduce
  // over hidden_dim
  bf16 *w_mlp1;  // gate_up_proj (n_layers, n_experts, 2 * intermediate_dim,
                 // hidden_dim)
  bf16 *w_mlp2;  // down_proj (n_layers, n_experts, hidden_dim, intermediate_dim)
  bf16 *b_mlp1;  // gate_up proj (n_layers, n_experts, 2 * intermediate_dim)
  bf16 *b_mlp2;  // down_proj (n_layers, n_experts, hidden_dim)
  // final norm [norm.scale]
  float *rms_out_w;  // (hidden_dim, )
  // classifier weights for the logits [unembedding.weight]
  float *out;  // (vocab_size, hidden_dim) (out, in)
} DeviceTransformerWeights;

typedef RunState DeviceRunState;
