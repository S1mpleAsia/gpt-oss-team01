...
void Tensor::to_device(hipStream_t stream) {
  size_t N_ = num_elem();
  if (dtype == DType::FP32) {
    CHECK_HIP(hipMemcpyAsync(d_buf, buf, N_ * sizeof(float),
    ...
    CHECK_HIP(hipMemcpyAsync(d_buf, temp_bf16, N_ * sizeof(bf16),
                            hipMemcpyHostToDevice, stream));
    free(temp_bf16);
  }
}
...
void Tensor::from_device(hipStream_t stream) {
  size_t N_ = num_elem();
  if (dtype == DType::FP32) {
    CHECK_HIP(hipMemcpyAsync(buf, d_buf, N_ * sizeof(float),
                            hipMemcpyDeviceToHost, stream));
  } else {
    // Convert and copy for BF16
    bf16* temp_bf16 = (bf16*)malloc(N_ * sizeof(bf16));
    CHECK_HIP(hipMemcpyAsync(temp_bf16, d_buf, N_ * sizeof(bf16),
                            hipMemcpyDeviceToHost, stream));
...
TensorI32::TensorI32(const vector<size_t> &shape_) 
...
void TensorI32::to_device(hipStream_t stream) {
  size_t N_ = num_elem();
  CHECK_HIP(hipMemcpyAsync(d_buf, buf, N_ * sizeof(int),
                          hipMemcpyHostToDevice, stream));
}

void TensorI32::from_device(hipStream_t stream) {
  size_t N_ = num_elem();
  CHECK_HIP(hipMemcpyAsync(buf, d_buf, N_ * sizeof(int),
                          hipMemcpyDeviceToHost, stream));
}
