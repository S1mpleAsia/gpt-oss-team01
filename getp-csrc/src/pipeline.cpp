...
PipelineEach::PipelineEach(const vector<size_t> &shape_, hipStream_t stream, int gpu_id,
                           int size_buf_, DType::Type dtype)
  ...
  x_buf = new Tensor(buffer_shape, stream, dtype);
...
void PipelineEach::enqueueElem(Tensor *x, int sent_gpu_id, hipEvent_t x_ready,
  ...
  CHECK_HIP(hipMemcpyPeerAsync((char *)x_buf->d_buf + offset, gpu_id, x->d_buf, sent_gpu_id,
                               slot_size * x_buf->get_dtype_size(), recv_stream));
...
void PipelineEach::dequeue(Tensor *x, hipStream_t stream) {
  ...
  CHECK_HIP(hipMemcpyAsync(x->d_buf, (char *)x_buf->d_buf + offset,
                           slot_size * x_buf->get_dtype_size(), hipMemcpyDeviceToDevice, stream));
