...
float *forward_gpu_20b_batched(int *tokens, int pos, int cur_batch_size, int flow_id) {
  ...
  for (int l = 0; l < p->n_layers; l++) {
    // attention rmsnorm
    rmsnorm_batched(rs_now->x, weights_now->rms_attn_w, rs_now->t, 1ll * l, false, false, 1e-5f,
                    stream);
    ...
    // QKV projection
    qkv_gemm_batched_v2(rs_now->t, weights_now->w_qkv, weights_now->b_qkv, rs_now->qkv, 1ll * l,
                        false, false, stream);  // This kernel diverges the most

    qkv_split_rope_fused(rs_now->qkv, rs_now->q, rs_now->key_cache, rs_now->value_cache,
                         rs_now->cos_tensor, rs_now->sin_tensor, p->head_dim, p->n_attn_heads,
    ...
    single_query_attn_flash_batched(rs_now->q, rs_now->key_cache, rs_now->value_cache, rs_now->mask,
                                    weights_now->attn_sinks, rs_now->tb, p->head_dim,
                                    p->n_attn_heads, kv_mul, kv_dim, p->seq_len, p->sliding_window,
                                    pos, 1ll * l, false, false, false, false, false, stream);

    // final matmul to get the output of the attention
    attn_out_project_batched_v2(rs_now->tb, weights_now->w_o, weights_now->b_o, rs_now->tb2,
                                1ll * l, false, false, stream);
    ...
    residual_rmsnorm_batched(rs_now->tb2, rs_now->x, weights_now->rms_ffn_w, rs_now->t, 1ll * l,
                             1e-5f, stream);
    ...
    moe_init_buffers_hip(rs_now->e_agg, rs_now->mlp1_out, rs_now->gate_up, rs_now->tb3,
                         ...
                         BATCH_SIZE, p->hidden_dim, stream);

    moe_build_offsets_hip(rs_now->topk_i, rs_now->sorted_pair_ids, rs_now->expert_offsets,
                          BATCH_SIZE, p->experts_per_token, p->n_experts, stream);

    moe_pack_inputs_hip(rs_now->t, rs_now->sorted_pair_ids, rs_now->x_packed, BATCH_SIZE,
    ...
    moe_swiglu_hip(rs_now->mlp1_out, rs_now->gate_up, BATCH_SIZE, p->experts_per_token,
    ...
    // residual connection
    add_vector_batched(rs_now->x, rs_now->e_agg, false, false, false,
  ...
  for (int pipeline_id = 0; pipeline_id < PP; pipeline_id++) {
    ...
    for (int l = 0; l < p->n_layers / PP; l++) {
      ...
      single_query_attn_batched(rs_now->q, rs_now->key_cache, rs_now->value_cache, rs_now->mask,
                                weights_now->attn_sinks, rs_now->tb, p->head_dim, p->n_attn_heads,
                                kv_mul, kv_dim, p->seq_len, p->sliding_window, pos, 1ll * l, false,
      ...
      // Select top-k experts
      topk_softmax_batched(rs_now->router_score, rs_now->topk_v, rs_now->topk_i, false, false,
                           false, stream);
...
#ifdef DEBUG
      if (flag)
        rs_now->sorted_pair_ids->printDebug("rs_now->sorted_pair_ids", tp_rank, 0, stream);
#endif
...
#ifdef DEBUG
      if (flag)
        rs_now->sorted_pair_ids->printDebug("rs_now->sorted_pair_ids pack inputs", tp_rank, 0,
                                            stream);
#endif
