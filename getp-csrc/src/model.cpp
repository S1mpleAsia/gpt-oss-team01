...
int *forward_gpu_20b_batched(int *tokens, int pos, int cur_batch_size, int flow_id) {
  ...
  classifier_gemm_batched_v2(weights_now->out, rs_now->x, rs_now->logits, cur_batch_size, false, false, stream);

  max_logits_batched(rs_now->logits, rs_now->logits_max, rs_now->logits_id, cur_batch_size, 0, false, false, true, stream);
...
int *forward_gpu_120b_batched(int *tokens, int pos, int cur_batch_size, int flow_id, int tp_rank,
  ...
  if ((pp_rank + 1) % PP == 0) {
    ...
    max_logits_batched(
      rs_now->tmp_logits, rs_now->logits_max, rs_now->logits_id,
      cur_batch_size, tp_rank, false, false, false, stream
    );

    // printf("Before all gather logits\n");
    // fflush(stdout);
    // all reduce here
    all_gather_logits_id(
    ...
    if (tp_rank == 0) {
      ...
      // fflush(stdout);
      reduce_logits_batched(
        rs_now->logits_max_total, rs_now->logits_id_total, rs_now->logits_max,
