...
void our_init_weights(TransformerWeights *w, Config *p, OurTransformerWeights *weights,
...

  {
    ...
    weights->out = new Tensor({hidden_dim, vocab_size}, 0, DType::BF16);
...
void our_init_run_state(RunState *s, Config *p, OurRunState *rs, int device_id) {
  ...
  rs->logits = new Tensor({BATCH_SIZE, (size_t)p->vocab_size});
  ...
  rs->mask = new Tensor({BATCH_SIZE, (size_t)p->seq_len, (size_t)p->seq_len});
