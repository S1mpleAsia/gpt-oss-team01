...
#endif
#ifndef ELEM_BYTES
#define ELEM_BYTES 2      // bytes per element
#endif
...
// Computes partial attention results
template <int TILE_TOKENS, int THREADS, int D, int G, int LOAD_ELEMS>
__global__ FA_WAVES_PER_EU __launch_bounds__(THREADS)
...
void single_query_attn_flash_batched(Tensor *q, Tensor *K_cache, Tensor *V_cache, Tensor *mask,
    ...
    if (head_dim != 64) {
        fprintf(stderr, "[FlashDec] This kernel only supports head_dim 64, got %d.\n", head_dim);
        abort();
    }
...
void flash_attn_batched_v2(Tensor *q, Tensor *key_cache, Tensor *value_cache, Tensor *mask,
  ...
  constexpr int tile_num_queries = 8;
  constexpr int tile_num_kv = 128;
  ...
  size_t shmem_bytes = 1ll * tile_num_queries * head_dim * sizeof(float) +
                       1ll * 2 * tile_num_kv * head_dim * sizeof(bf16) * 2 +
