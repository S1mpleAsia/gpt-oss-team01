  ...
  int t_begin, int t_end, int B, int C)
{
  ...
  for (int i_out = (threadIdx.x << 1); i_out < D; i_out += (blockDim.x << 1)) {
    ...
    for (int t = 0; t < T; ++t) {
      ...
      const uint32_t vp = ld_u32(reinterpret_cast<const uint32_t*>(
...
__global__ void fd_reduce_kernel_stable_v2(
  const float *__restrict__ partial_max,  // [B, n_q, C]
  const float *__restrict__ partial_sum,  // [B, n_q, C]
  const float *__restrict__ partial_num,  // [B, n_q, C, D]
  ...
  bool tb_from_device, hipStream_t stream)
{
  ...
  fd_reduce_kernel_stable_v2<64>
      <<<grid_r, block_r, 0, stream>>>(
        d_pmax, d_psum, d_pnum, S_ptr,
        (float*)tb->d_buf, n_q, C, B);
  CHECK_HIP(hipGetLastError());
