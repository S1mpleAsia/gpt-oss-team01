...
template <int BM, int BN, int BK, int TM, int TN, int BLOCK_THREADS>
__global__ __launch_bounds__(BLOCK_THREADS) void gemm_mfma_moe_v2_bf16(
...
template <int BM, int BN, int BK, int TM, int TN, int BLOCK_THREADS>
__global__ __launch_bounds__(BLOCK_THREADS) void gemm_mfma_moe_fused_bf16(
...
#pragma unroll
  ...
  for (int linear = threadIdx.x; linear < m_valid * n_valid_pairs; linear += BLOCK_THREADS) {
    ...
    C[(size_t)row_global * H + out_col_global] = (bf16)outv;
  }
}
