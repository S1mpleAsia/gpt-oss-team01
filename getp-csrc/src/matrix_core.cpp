  ...
  for (int i = 0; i < A_VEC_PER_THR; i++) {
    ...
    if (g_row < M && (g_col + VEC_A_SIZE - 1) < K) {
      *reinterpret_cast<float4 *>(&As[r][c]) =
        *reinterpret_cast<const float4 *>(&A[(size_t)g_row * K + g_col]);
    } else {
#pragma unroll
      for (int j = 0; j < VEC_A_SIZE; j++) {
        As[r][c + j] = (g_row < M && (g_col + j) < K) ? A[(size_t)g_row * K + (g_col + j)] : 0.0f;
      }
    }
  }
...
#pragma unroll
      for (int i = 0; i < A_VEC_PER_THR; i++) {
        ...
        if (g_row < M && (g_col + VEC_A_SIZE - 1) < K) {
          ...
          float tmp[VEC_A_SIZE];
#pragma unroll
          for (int j = 0; j < VEC_A_SIZE; j++) {
            tmp[j] = (g_row < M && (g_col + j) < K) ? A[(size_t)g_row * K + (g_col + j)] : 0.0f;
          }
...
#pragma unroll
      for (int i = 0; i < A_VEC_PER_THR; i++) {
        ...
        int r = elem_idx / BK;
        int c = elem_idx % BK;
        *reinterpret_cast<float4 *>(&As[r][c]) = regA[i];
      }
...
#pragma unroll
  for (int i = 0; i < A_VEC_PER_THR; i++) {
    int vec_idx = tid + i * BLOCK_THREADS;
    int elem_idx = vec_idx * VEC_A_SIZE;
    int r_local = elem_idx / BK;
    int c = elem_idx % BK;
    int g_row_local = r_local;
    int g_row_global = block_row_global_start + g_row_local;
    int g_col = c;
    if (g_row_local < M && (g_col + VEC_A_SIZE - 1) < K) {
      *reinterpret_cast<float4 *>(&As[r_local][c]) =
        *reinterpret_cast<const float4 *>(&A[(size_t)g_row_global * K + g_col]);
    } else {
#pragma unroll
      for (int j = 0; j < VEC_A_SIZE; j++) {
        As[r_local][c + j] =
...
#pragma unroll
      for (int i = 0; i < A_VEC_PER_THR; i++) {
        ...
        int r_local = elem_idx / BK;
        int c = elem_idx % BK;
        *reinterpret_cast<float4 *>(&As[r_local][c]) = regA[i];
      }
