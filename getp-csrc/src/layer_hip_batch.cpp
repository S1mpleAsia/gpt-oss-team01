...
void attn_out_project_batched_v2(Tensor *tb,         // Shape: [batch_size, n_attn_heads * head_dim]
                                 ...
                                 long long layer_offset, bool tb_to_device, bool y_from_device,
                                 hipStream_t stream) {
  GpuTimer timer("attn_out_project_v2");
  if (tb_to_device) {
    tb->to_device(stream);
  }
...
static inline void moe_mlp2_batched(Tensor *gate_up, Tensor *w_mlp2, Tensor *b_mlp2, Tensor *tb3,
                                    TensorI32 *topk_idx, int k, long long layer_offset,
                                    hipStream_t stream) {
  ...
  const bf16 *b_mlp2_ptr =
    (const bf16 *)b_mlp2->d_buf + (size_t)layer_offset * n_experts * hidden_dim;
