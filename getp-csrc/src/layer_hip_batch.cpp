...
__global__ void gemm_kernel_batched(const float *W, const float *x, const float *bias, float *out,
...
__global__ void batched_matmul_kernel(const float *W, const float *x, const float *bias, float *out,
                                      int out_features, int in_features, int batch_size) {
...
void router_gemm_batched(const Tensor *w_router,  // Shape: [n_experts, hidden_dim]
  ...
  if (t_to_device) {
    ...
    dim3 grid_dim((n_experts + WARPS_PER_BLOCK - 1) / WARPS_PER_BLOCK, batch_size);
...
// ---------- MoE apply TopK  ----------
__global__ void SwiGLU_kernel_batched(const float *interleaved_in, float *swiglu_out, int inter_dim,
                              float clamp_limit) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < inter_dim) {
 ...
 */
__global__ void moe_matmul_kernel_bf16_weights_batch(const bf16 *W, const float *x, const bf16 *bias,
                                                      float *out, const int *topk_idx, int batch_size, int k,
 ...
 */
__global__ void batched_weighted_accumulate_kernel_batch(const float *expert_outputs,
...
void moe_apply_topk_batched(
  ...
  bool topk_vals_to_device, bool e_agg_from_device, hipStream_t stream) {
    ...
    // Extract dimensions from tensor shapes
    const int batch_size = t->shape[0];
    ...
    size_t total_threads_mlp1 = (size_t)batch_size * k * 2 * inter_dim;
    moe_matmul_kernel_bf16_weights_batch<<<(total_threads_mlp1 + DEFAULT_BLOCK_SIZE - 1) / DEFAULT_BLOCK_SIZE,
        ...
        batch_size, k, 2 * inter_dim, hidden_dim, false);
    ...
    size_t swiglu_threads = (size_t)batch_size * k * inter_dim;
    SwiGLU_kernel_batched<<<(swiglu_threads + DEFAULT_BLOCK_SIZE - 1) / DEFAULT_BLOCK_SIZE,
                   DEFAULT_BLOCK_SIZE, 0, stream>>>(mlp1_out_ptr, gate_up_ptr, swiglu_threads, clamp_limit);

    // --- Step 3: Second GEMM (Batched) ---
    // y = W2 * swiglu + b2 -> write to tb3
    size_t total_threads_mlp2 = (size_t)batch_size * k * hidden_dim;
