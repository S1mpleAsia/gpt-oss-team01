...
void RMSNorm(const Tensor *x /*[hidden]*/, const Tensor *scale /*[n_layers, hidden]*/,
...
void Linear(const float *x, size_t in_dim, const float *W, const float *b, size_t out_dim,
            float *y) {
...
void QKVProject(const Tensor *x /*[hidden]*/, const Tensor *W_qkv /*[(n_q+2*n_kv)*hd, hidden]*/,
                const Tensor *b_qkv, Tensor *qkv /*[(n_q+2*n_kv)*hd]*/, long long layer_offset) {
...
void SplitQKV(const Tensor *qkv, int head_dim, int n_q, int n_kv, Tensor *q /*[n_q*hd]*/,
              Tensor *k /*[n_kv*hd]*/, Tensor *v /*[n_kv*hd]*/) {
  size_t offset = 0;
 ...
 * size = seq_len * (head_dim / 2).
 */
void RopePrecomputeCS(Config *p, Tensor *cos_all_out, Tensor *sin_all_out, hipStream_t stream) {
 ...
 */
void ApplyRotary(Tensor *x /*[n_heads*hd]*/, const Tensor *cos_table /*[seq_len*hd/2]*/,
                 const Tensor *sin_table /*[seq_len*hd/2]*/, int n_heads, int head_dim, int pos) {
  int half = head_dim / 2;
...
void AttnScoresAllHeads(Tensor *key_cache, Tensor *q, Tensor *att, Tensor *attn_sinks, Tensor *mask,
                        long long loff_one, long long layer_offset, int attn_heads, int kv_mul,
                        int head_dim, int kv_dim, int seq_len, int sliding_window, int pos) {
...
void AttnWeightedSumAllHeads(Tensor *value_cache, Tensor *q, Tensor *att, Tensor *tb,
                             long long loff, int attn_heads, int kv_mul, int head_dim, int kv_dim,
                             int seq_len, int pos) {
...
void AttnOutProject(const Tensor *tb /*[n_q*hd]*/, const Tensor *W_o /*[hidden, n_q*hd]*/,
                    const Tensor *b_o, Tensor *y /*[hidden]*/, long long layer_offset) {
...
void RouterScores(const Tensor *t /*[hidden]*/, const Tensor *W_router /*[n_experts, hidden]*/,
                  const Tensor *b_router, Tensor *r /*[n_experts]*/, long long layer_offset) {
...
void SwiGLU(const Tensor *gate /*[d]*/, const Tensor *up /*[d]*/, float clamp_limit,
            Tensor *out /*[d]*/) {
...
void ExpertFFN1_Total(const Tensor *t, const Tensor *W1, const Tensor *b1, Tensor *z,
                      const TensorI32 *topk_i, long long layer_offset) {
...
void ExpertFFN2_Total(const Tensor *swiglu, const Tensor *W2, const Tensor *b2, Tensor *y,
                      const TensorI32 *topk_i, long long layer_offset) {
...
void Classifier(const Tensor *x /*[hidden]*/, const Tensor *W_out /*[vocab, hidden]*/,
                Tensor *logits /*[vocab]*/) {
  Linear(x->buf, x->num_elem(), W_out->buf, nullptr, logits->num_elem(), logits->buf);
}
